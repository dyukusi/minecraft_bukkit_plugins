...
public class Notificator extends BukkitRunnable {
	...
	public void run() {
		...
		if (!plugin.isSame_area_with_last_time(player)) {
			...
			} else if (current_area_name
					.equals(Biome.EXTREME_HILLS_PLUS.name())) {
				current_area_name = Biome.EXTREME_HILLS.name();
			}

			player.removeMetadata("area", plugin);
			player.setMetadata("area", new FixedMetadataValue(plugin,
					current_area_name));
			...
			if (in_custom_area) {
				CustomAreaInfo info = plugin
						.get_customarea_info(current_area_name);

				if (info.get_as_subtitle()) {
					title = new Title(info.get_area_name(),
							info.get_creater_name());
					title.setTitleColor(ChatColor.RED);
					title.setSubtitleColor(ChatColor.AQUA);
				} else {
					title = new Title(info.get_area_name(), "by "
							+ info.get_creater_name());
				...
				title = new Title(Language_EtoJ.valueOf(current_area_name)
						.get_inJapanese(), "< " + current_area_name + " >");
